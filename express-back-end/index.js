...
app.post('/updateTeam/:Id', async (req, res) => {
    ...
    if(validKey)
    {
        ...
        res.json({Message:'Successfully updated team standing', Team: await hockeyDB.DAL.UpdateTeamGames(id, updatedTeam)})
    }
    else
    {
        return res.json({Message: "New Team must have some kind of a name"})
...
app.get('/removeTeam/:Id', async (req, res) => {
    ...
    if(validKey)
    {
        res.json({Message: "Team was deleted from database" , Team: await hockeyDB.DAL.DeleteTeam(req.params.Id)})
    }
    else
    {
...
app.post("/newUser/", async (req, res) => {
    ...
    else
    {
        ...
        else
        {
            ...
            res.json({Message: email + " wasn''t found creating a new user", User: user})
        }
    }
